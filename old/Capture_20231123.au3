...
; 23/5/10  DAYS全画面中断処理復帰・DAYS中央・ヤンチャンウェブ中央
; 23/06/06  DAYSの雑誌と単話を統合・ヤンマガWEBを追加
; 23/11/23  ebook no renzoku syori henkou
Const $LASTUPDATE  =  "2301123"
...
;=================================================
; EBOOK 特集　次巻クリック座標,(中心)ボタン色(misiyou)
global $DEB_NEXT[2]
global $DEB_COLOR ; misiyou

;=================================================
; チャンピオンWEB　終端判定
;　中心座標　左右ページオフセット　左右判定オフセット　上下判定オフセット
;  背景色　ホワイト色　ボタン色
global $D_CW_CENTER[2]
global $D_CW_PAGE_OFFSET
global $D_CW_JDG_LR_OFFSET
global $D_CW_JDG_UD_OFFSET
global $D_CW_CL_BG
global $D_CW_CL_WHITE
global $D_CW_CL_BTN
...
;				  endif
				  ...
				  if ( $NextMachi = true ) Then
				   ...
				   ;============================================
				  ; ページ中にカラー（広告？）があれば今ページと次ページの待ち時間延長5sec(単話時のみ)
				  if ( $DaysMode=1 ) then ; 四方時
			...
			;============================================
				  ...
				  $a[$XXX] = $D_CW_CENTER[$XXX] - $D_CW_PAGE_OFFSET -  $D_CW_JDG_LR_OFFSET
				  $b[$XXX] = $D_CW_CENTER[$XXX] - $D_CW_PAGE_OFFSET +  $D_CW_JDG_LR_OFFSET
				  $c[$XXX] = $D_CW_CENTER[$XXX] - $D_CW_PAGE_OFFSET -  $D_CW_JDG_LR_OFFSET
				  $d[$XXX] = $D_CW_CENTER[$XXX] - $D_CW_PAGE_OFFSET +  $D_CW_JDG_LR_OFFSET
				  $a[$YYY] = $D_CW_CENTER[$YYY] +  $D_CW_JDG_UD_OFFSET
				  $b[$YYY] = $D_CW_CENTER[$YYY] +  $D_CW_JDG_UD_OFFSET
				  $c[$YYY] = $D_CW_CENTER[$YYY] -  $D_CW_JDG_UD_OFFSET
				  $d[$YYY] = $D_CW_CENTER[$YYY] -  $D_CW_JDG_UD_OFFSET

				  $e[$XXX] = $D_CW_CENTER[$XXX] + $D_CW_PAGE_OFFSET -  $D_CW_JDG_LR_OFFSET
				  $f[$XXX] = $D_CW_CENTER[$XXX] + $D_CW_PAGE_OFFSET +  $D_CW_JDG_LR_OFFSET
				  $g[$XXX] = $D_CW_CENTER[$XXX] + $D_CW_PAGE_OFFSET
				  $h[$XXX] = $D_CW_CENTER[$XXX] + $D_CW_PAGE_OFFSET
				  $e[$YYY] = $D_CW_CENTER[$YYY]
				  $f[$YYY] = $D_CW_CENTER[$YYY]
				  $g[$YYY] = $D_CW_CENTER[$YYY]  +  $D_CW_JDG_UD_OFFSET
				  $h[$YYY] = $D_CW_CENTER[$YYY]  -  $D_CW_JDG_UD_OFFSET

; MouseMove($e[$XXX],$e[$YYY])
; MouseMove($f[$XXX],$f[$YYY])

				  if  ( _
						   ( _
							  ($D_CW_CL_BG = PixelGetColor ( $a[$XXX],$a[$YYY])) And _
							  ($D_CW_CL_BG = PixelGetColor ( $b[$XXX],$b[$YYY])) And _
							  ($D_CW_CL_BG = PixelGetColor ( $c[$XXX],$c[$YYY])) And _
							  ($D_CW_CL_BG = PixelGetColor ( $d[$XXX],$d[$YYY])) _
						   ) And ( _
							  ($D_CW_CL_BTN = PixelGetColor ( $e[$XXX],$e[$YYY])) And _
							  ($D_CW_CL_BTN = PixelGetColor ( $f[$XXX],$f[$YYY])) And  _
							  ($D_CW_CL_WHITE = PixelGetColor ( $g[$XXX],$g[$YYY])) And _
							  ($D_CW_CL_WHITE = PixelGetColor ( $h[$XXX],$h[$YYY])) _
						   ) _
						) Then
						   ; 左　四箇所全部黒　且つ
						   ; 右　ボタン黒且つ　上下白
   ...
   ;============================================
	...
	;============================================
						  ; ■DAYS連続　
							MsgBOX( 0, "PAUSE", "ヤンマガWEB Auto End Detect" )
							$startstop=0
   ;============================================
   ...
   ;============================================
					;  ■チャンピオンWEB連続
							MsgBOX( 0, "PAUSE", "ChampionWeb Auto End Detect" )
							$startstop=0
   ;============================================
...
;============================================
	Case $EndMode = $EM_DAYS
		; 左キー
		$PageKind = 3
		MsgBOX( 0, "PAUSE", "★コミックDAYS" )
	Case $EndMode = $EM_YMWEB
		; 左キー
		$PageKind = 3
		MsgBOX( 0, "PAUSE", "★ヤンマガWEB" )
;============================================
	Case $EndMode = $EM_EBOOK
		; 左キー
		$PageKind = 3
...
EndFunc
 ...
 Func Proc_InitVar()
   ...
   $DYM_ENDCOLOR = call( "Func_GetParaValue", "capture.ini","DYM_ENDCOLOR")
	...
	$D_CW_CENTER[$XXX] = call( "Func_GetParaValue", "capture.ini","D_CW_CENTER_X")
	$D_CW_CENTER[$YYY] = call( "Func_GetParaValue", "capture.ini","D_CW_CENTER_Y")
	$D_CW_PAGE_OFFSET= call( "Func_GetParaValue", "capture.ini","D_CW_PAGE_OFFSET")
	$D_CW_JDG_LR_OFFSET = call( "Func_GetParaValue", "capture.ini","D_CW_JDG_LR_OFFSET")
	$D_CW_JDG_UD_OFFSET = call( "Func_GetParaValue", "capture.ini","D_CW_JDG_UD_OFFSET")
	$D_CW_CL_BG = call( "Func_GetParaValue", "capture.ini","D_CW_CL_BG")
	$D_CW_CL_WHITE = call( "Func_GetParaValue", "capture.ini","D_CW_CL_WHITE")
	$D_CW_CL_BTN = call( "Func_GetParaValue", "capture.ini","D_CW_CL_BTN")
EndFunc
...
;MsgBOX( 0, "Two", $xTpos )
;MsgBOX( 0, "Three", $ypos )
;mousemove( $xOpos,$ypos )
;mousemove( $xTpos,$ypos )
   if  ( _
...
; コミックDAYSのページ内容が白黒(True)でない（False）　単話のみ判定
; 判定エリア＝左右　対角線
Func   DaysBackColor( )
   local $xx , $yy
   local $startX
