...
func main()
   ...
   ;============================================
				...
				$Asum = call( "AreaSum", $D_CHECK_CENTER[$XXX],$D_CHECK_CENTER[$YYY] )
				if ($NEXTPOS<1) Then
				   MsgBOX( 0, "PAUSE", "Jump Shelf End Detect" )
				   $startstop=0
			   else
				...
				; 次本ロード待ち( ジャンプの初期ロード待ちはアニメじゃなくプログレスバーであり、１秒程度は止まることもあるので組わせて確認)
				call( "EscapeNotEqual", $Asum ,  $D_CHECK_CENTER[$XXX],$D_CHECK_CENTER[$YYY] )
			   while 1
				  call( "EscapeEqualSec", $D_CHECK_CENTER[$XXX],$D_CHECK_CENTER[$YYY],1 )
   ...
   ;============================================
			   ...
			   ;  次の話クリック
				...
				MouseClick( "left",$D_YJ_NEXT[$XXX],$D_YBJ_NEXT,1 )
				sleep(5000)
...
Func Proc_InitVar()
	...
	$D_YJ_NEXT[$YYY] = call( "Func_GetParaValue", "setting.ini","DYJ_NEXT_YA")
	$D_YBJ_NEXT = call( "Func_GetParaValue", "setting.ini","DYJ_NEXT_YB")
...
;========================================================================
;　 N番目の2次元本棚座標（左上から右へNカウント）を返す　N（MAX）=　列数*行数　になる。
