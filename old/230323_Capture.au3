...
; ebooks 次巻開始仕様変更
;  21/09/23
; NOX 削除　順番入れ替え　次巻処理中のカウンタ一時停止
;  21/09/28
; Ebookだけ長く待つ＋ヤンマガweb連続対応(暫定)
;  21/10/29
; 背景色白（ebook）の待ち時間延長
;  21/12/04
; キー入力を止めないように修正（フック時に再スロー）
; 各種背景色時の待ち時間を全て延長＆背景色検出時にアラーム鳴らす
;  22/02/04
; 白色背景時の検出条件のみ変更＆待ち時間を再度延長
...
; 23/3/24  DAYS 色付き検出後待ち時間延長/ 全画面中断時リカバリ処理暫定削除
Const $LASTUPDATE  =  "230324"
