...
; 26/06/14  HotKey問題対応（ALT付きへ）/ゴラク、チャンピオン削除　BookWalker終端再対応
; 26/06/18  AI debug
...
; ALT+E: モード変更(ページ遷移方法も同時に変更)　
; ALT+W: ページ送り待ち時間変更　7sec(default)/14sec
...
;▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲
   ...
   while 1
			...
			while 1
			   if $startstop=1 Then
				  ...
				  If $EndMode = $EM_KOMIFLO Then
					  Sleep(3000) ; KOMIFLO時はALTキーによる画面変化が収まるまで3秒待機
				  Else
					  Sleep(1000) ; その他のモードは1秒待機
				   EndIf
				  exitLoop
			   endif
			WEnd
         ...
         ; 【デバッグ追加】取得したサム値と履歴の出力★★★★★★
		 WriteDebugLog("Mekuri完了 -> 最新(END):" & $ENDsum & " | 前(bef0):" & $befsum[0] & " | 2回前(bef1):" & $befsum[1] & " | 3回前(bef2):" & $befsum[2])
...
Func Proc_Mekuri()
   ...
   ; ★修正1: ページめくり全体のタイムアウト用タイマーを【ここで】リセットする
    call( "Proc_ResetTimer", 0 )

    ; ★修正2: キャプチャキー押下後、画面のフラッシュや保存ダイアログが消えるのを待ってから基準サムを取得する
    sleep(500) ; 300から500に増やして余裕を持たせる
	...
	while 1
			   ...
			   $Csum = call( "KoteiSum")
				if  ( $Bsum <> $Csum ) or _
				   ...
				   ; ▲▽（背景色による状態B継続）もしくは（状態B’アニメーション）▲▽
					; ◆Bサム値　更新　動きを継続して確認
						$Bsum = $Csum
						; ★修正3: デフォルトのタイムアウト(秒)を設定。未定義(0)による即時脱出を防ぐ
						$TO =3
		...
		Else
			...
			; タイムアウトによる脱出
            ; ★修正4: ページが動き出すまでの固定タイムアウト値（ここでは5秒）を明記
    ...
    ; 【デバッグ追加】めくり判定内部のパラメータを出力★★★★★★
	WriteDebugLog("Mekuri詳細 -> 初期サム(Asum):" & $Asum & " | 最終サム(Bsum):" & $Bsum & " | ExitFLG:" & $ExitFLG & " | ローディング延長(TO):" & $TO)
...
Func Proc_BookwalkerNext()
	  ...
	  $result[1] =  call( "Func_JudgeX_Color", $DBW_NEXT_COL[1] , $DBW_NEXT_POS[$XXX],430,850 )
	   if  ((-1 <> $result[0] ) Or  (-1 <> $result[1] )) Then
             ...
             ; 【デバッグ追加】誤検知した座標をログ出力★★★★★★
             WriteDebugLog("BWボタン誤検知 -> X座標: " & $DBW_NEXT_POS[$XXX] & " | 検出Y1: " & $result[0] & " | 検出Y2: " & $result[1])
...
;    Local $hFile = FileOpen("debug_log.txt", 1) ; 1 = 追加書き込みモード
;    If $hFile <> -1 Then
